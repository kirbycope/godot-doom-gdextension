...
that builds, demos and tests the addon; the addon itself is `addons/pure_doom/`, and that is the only folder
...
`res://addons/pure_doom/scenes/demo/demo.tscn` is the project's main scene. Run the project and DOOM boots
...
See `addons/pure_doom/README.md` for the node's properties, signals, methods and the controls card.

## Building

Prebuilt Windows, macOS and web libraries are committed in `addons/pure_doom/bin/`, so the demo runs from a
...
command lines are in `addons/pure_doom/README.md`.
...
The wrapper in `addons/pure_doom/src/`, the GDScript, scenes, resources and tests are MIT (`LICENSE`).

The bundled engine is not. `addons/pure_doom/thirdparty/PureDOOM.h` is GPL 2.0, so the libraries in
`addons/pure_doom/bin/` and anything you ship that links them are bound by that licence; its terms are in
`addons/pure_doom/thirdparty/LICENSE`. `addons/pure_doom/assets/doom1.wad` is id Software's freely
redistributable shareware IWAD. The full credits table is in `addons/pure_doom/README.md`.
